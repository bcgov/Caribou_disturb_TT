...
# read in the csv.s created in script 1: 
...
#2)  Rail  # no rail in the herd boundaries 
        
# 3) dams
r.dam.sf <- st_read(dsn=Base,layer="dams_clip" ) # multipol
...
r.dams$Length_m = as.numeric(st_length(r.dams))    # calculate the length
r.dams.df = data.frame(r.dams)        # calculate the length per range 
  ...
  out.l= st_union(out.l,out.t) ; plot(st_geometry(out.l))
         

# 1) pipeline (length)
r.pipe <- st_read(dsn=Base,layer="Pipeline_clip") # multistring 
      r.pipe <- st_zm(r.pipe,drop = TRUE)
      r.pipe = st_transform(r.pipe,3005)
      r.pipe<- st_cast(r.pipe, "MULTILINESTRING")
      ...
      r.pipe.int<- st_cast(r.pipe.int, "MULTILINESTRING")
      st_is_valid(r.pipe.int)                       # check valid geometry
      r.pipe.int$Length_m = as.numeric(st_length(r.pipe.int))
      r.pipe.int.df = data.frame(r.pipe.int)        # calcaulte area
      r.pipe.int.df.out  = r.pipe.int.df %>% 
        group_by(SiteName,V17_CH) %>% 
        summarise(R_Pipe_Length_m = sum(Length_m))
      
      out.sf= r.pipe.int
      out.sf = st_cast(out.sf,"LINESTRING")
